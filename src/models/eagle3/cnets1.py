    ...
    def forward(
        ...
        if (
            ...
            if not _FLASH_ATTN_WARNING_EMITTED:
                logger.warning("Using flash_attn_func for Eagle3 draft attention.")
        ...
        if use_eagle_fast_attention and attention_impl in {"sdpa", "flash_attention_2"}:
            if attention_impl == "flash_attention_2" and self.flash_attn_func is None:
                ...
                if not _FLASH_ATTN_FALLBACK_WARNING_EMITTED:
                    logger.warning("flash_attn is not installed; falling back to PyTorch SDPA for Eagle3 draft attention.")
...
class Model(nn.Module):
    def __init__(self, config, load_emb=False, path=None, bias=True, total_tokens=63, depth=5, top_k=8):
        ...
        self.depth = depth
        self.draft_sliding_window = None
    ...
    def reset_kv(self):
        self.stable_kv = None
        self.stable_input_length = 0

    def _trim_stable_kv(self, keep_length):
        if self.stable_kv is None or keep_length is None:
            return
        first_cache = self.stable_kv[0][0]
        current_length = (
            int(first_cache.current_length.item())
            if hasattr(first_cache, "current_length")
            else int(first_cache.shape[2])
        )
        if keep_length <= 0:
            for layer_cache in self.stable_kv:
                for cache in layer_cache:
                    if hasattr(cache, "current_length"):
                        cache.current_length.zero_()
            if not hasattr(first_cache, "current_length"):
                self.stable_kv = tuple(
                    tuple(cache[:, :, :0, :] for cache in layer_cache)
                    for layer_cache in self.stable_kv
                )
            return
        if current_length <= keep_length:
            return
        if hasattr(first_cache, "current_length"):
            start = current_length - keep_length
            for layer_cache in self.stable_kv:
                for cache in layer_cache:
                    cache.data[:, :, :keep_length, :].copy_(
                        cache.data[:, :, start:current_length, :]
                    )
                    cache.current_length.fill_(keep_length)
        else:
            self.stable_kv = tuple(
                tuple(cache[:, :, -keep_length:, :].contiguous() for cache in layer_cache)
                for layer_cache in self.stable_kv
            )
    ...
    def topK_genrate(self, hidden_states, input_ids, head, logits_processor):
        ...
        input_ids = input_ids.to(hidden_states.device)
        full_input_length = input_ids.shape[1]
        sliding_window = self.draft_sliding_window
        if sliding_window is not None:
            sliding_window = int(sliding_window)
            if sliding_window > 0 and self.stable_kv is None and input_ids.shape[1] > sliding_window:
                input_ids = input_ids[:, -sliding_window:]
                hidden_states = hidden_states[:, -sliding_window:]
        ...
        if hasattr(self, "stable_kv") and self.stable_kv is not None:
            cached_input_length = getattr(self, "stable_input_length", self.stable_kv[0][0].shape[2])
            new_input_ids = input_ids[:, cached_input_length:]
            if sliding_window is not None and sliding_window > 0:
                keep_length = max(sliding_window - new_input_ids.shape[1], 0)
                self._trim_stable_kv(keep_length)
            out_hidden, past_key_values = self(hidden_states, input_ids=new_input_ids,
                                               past_key_values=self.stable_kv, use_cache=True)
        else:
            out_hidden, past_key_values = self(hidden_states, input_ids=input_ids, use_cache=True)
        self.stable_kv = past_key_values
        self.stable_input_length = full_input_length
        len_posi = self.stable_kv[0][0].shape[2]
