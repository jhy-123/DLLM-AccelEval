...
class Fast_dLLM_QwenForCausalLM:
    ...
    def batch_sample(
        ...
        return finished_samples, full_step, {
            "avg_q_part_step_time_ms": (
                decode_time_ms / full_step if full_step > 0 else None
            ),
            "avg_q_full_step_time_ms": (
                prefill_time_ms if prefill_step_count > 0 else None
            ),
            "init_time_ms": prefill_time_ms if prefill_step_count > 0 else None,
            "avg_q_full_step_count": float(prefill_step_count),
