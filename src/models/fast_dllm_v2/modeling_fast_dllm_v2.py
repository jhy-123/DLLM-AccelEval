...
_FA2_BLOCK_MASK_WARNING_EMITTED = False
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if self.training:
            ...
            attention_impl = getattr(self.config, "_attn_implementation", None) or getattr(self.config, "attn_implementation", None) or "sdpa"
            if attention_impl not in ALL_ATTENTION_FUNCTIONS:
                attention_impl = "sdpa"

            # Transformers FA2 expects a padding mask of shape [batch, seq] or packed position ids.
            # Fast-dLLM v2 uses a custom block-structured boolean attention mask of shape [q_len, kv_len]
            # during decode/prefill, which FA2 cannot consume correctly through the standard integration.
            if (
                attention_impl == "flash_attention_2"
                and attention_mask is not None
                and attention_mask.dim() == 2
                and attention_mask.shape != (query_states.shape[0], key_states.shape[2])
            ):
                global _FA2_BLOCK_MASK_WARNING_EMITTED
                if not _FA2_BLOCK_MASK_WARNING_EMITTED:
                    logger.warning(
                        "Falling back from flash_attention_2 to sdpa: Fast-dLLM v2 block decoding uses a custom 2D block attention mask that is incompatible with the standard Hugging Face FA2 integration."
                    )
                    _FA2_BLOCK_MASK_WARNING_EMITTED = True
                attention_impl = "sdpa"

            attention_interface = ALL_ATTENTION_FUNCTIONS[attention_impl]
...
class Fast_dLLM_QwenForCausalLM(Fast_dLLM_QwenPreTrainedModel):
